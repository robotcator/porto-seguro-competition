...
if os.path.exists('../input'):
    ...
    strdirectory = '../dropcal_one_hot'
gptrain, gptest, _ = GetData(strdirectory)

def get_data(data):
    v1 = get_GP1_features(data)
    v2 = get_GP2_features(data)
    # return pd.concat([v1, v2], axis=1)
    return v1 + v2
    # return v1

train_gp = get_data(gptrain)
test_gp = get_data(gptest)
...
if __name__ == '__main__':
    ...
    train_pred, test_pred, mean, std, full_score = five_fold_with_baging(train_gp, gptrain.target,
                            test_gp, train_id, test_id, metric, mdl, mdl_type=model['mdl_type'], seed=1024,
                            stratified=False, n_splits=model['split'], n_folds=model['fold'], n_bags=model['bag'],
                            early_stop=-1, verbose=False)
    write_csv(train_pred, test_pred, mdl_name=model['name']+'-gp', mean=mean, overall_mean=full_score, std=std, bag=-1)
